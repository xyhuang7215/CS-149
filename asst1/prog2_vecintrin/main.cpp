...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  //
  float clampVal = 9.999999f;
  __cs149_vec_float result;
  __cs149_vec_float value;
  __cs149_vec_int exp;
  __cs149_vec_int allZero = _cs149_vset_int(0);
  __cs149_vec_int allOne = _cs149_vset_int(1);
  __cs149_vec_float upper_bound = _cs149_vset_float(clampVal);
  ...
  for (i = 0; i + VECTOR_WIDTH <= N; i += VECTOR_WIDTH) {
    ...
    _cs149_vgt_int(needMul, exp, allZero, maskAll);

    result = _cs149_vset_float(1.f);
    while (_cs149_cntbits(needMul)) {
      _cs149_vmult_float(result, result, value, needMul);
      _cs149_vsub_int(exp, exp, allOne, needMul);
      _cs149_vgt_int(needMul, exp, allZero, needMul);
    }

    _cs149_vgt_float(clampMask, result, upper_bound, maskAll);
    _cs149_vset_float(result, clampVal, clampMask);
    _cs149_vstore_float(output + i, result, maskAll);
  }
