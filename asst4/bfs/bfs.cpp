...
void top_down_step_parallel(
    ...
    int* distances)
{
    int prevDis = distances[frontier->vertices[0]];
    int currDis = prevDis + 1;
...
// Since we will traverse every node each iteration,
// serial version is slow than parallel version even when frontier_size is small.
...
//             if (distances[incoming] == prevDis) {
//                 distances[node] = currDis;
    ...
    bool* visited)
{
    ...
    //  spilt all nodes into groups in advanced
    //  so that we can update local frontier
    ...
    for (int start_idx = 0; start_idx < g->num_nodes; start_idx += nodes_per_thread) {
        ...
        for (int node = start_idx; node < end_idx; node++) {
            ...
            for (int neighbor = start_edge; neighbor < end_edge; neighbor++) {
                ...
                // if (distances[incoming] != prevDis) continue;
